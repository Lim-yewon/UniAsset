...
export default function HomePage() {
  ...
  const rentalStats = useMemo(() => {
    ...
    const rented = rentables.filter((a) =>
      a.rentals?.some((r: any) => r.status === '대여중')
    ).length;
  ...
  const faultStats = useMemo(() => {
    const filtered = assets.filter(
      (a) => selectedDept === '전체' || a.department === selectedDept
    );
    ...
    const roomBreakdown = faultyAssets.reduce((acc, asset) => {
      ...
      const key = `${loc} ${roomNum}`.trim();
      acc[key] = (acc[key] || 0) + 1;
      return acc;
    }, {} as Record<string, number>);

    const sortedRooms = (Object.entries(roomBreakdown) as [string, number][]).sort(
      (a, b) => b[1] - a[1]
    );
  ...
  const summaryCards = [
    ...
    {
      label: '고정 자산',
      value: assets.filter((a) => !a.is_rentable).length,
      sub: '강의실 배치 기자재',
    ...
    {
      label: '대여 가능',
      value: rentalStats.available,
      sub: `전체 대여용 ${rentalStats.total}개 중`,
    ...
    <div className="max-w-5xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="md:col-span-1 bg-white p-6 rounded-2xl shadow-sm border border-slate-200 flex flex-col items-center justify-center">
          <h2 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4 w-full text-left">
            대여용 기자재 가동률
          </h2>
          <div
            className="relative w-32 h-32 rounded-full flex items-center justify-center mb-4"
            style={{
              background: `conic-gradient(#ef4444 ${rentalStats.rentedPercent}%, #22c55e ${rentalStats.rentedPercent}% 100%)`,
            }}
          >
          ...
          <div className="flex w-full justify-between text-sm px-2">
            <div className="text-center">
              ...
              <p className="font-bold text-slate-700">{rentalStats.total}</p>
            </div>
            <div className="text-center">
              <p className="text-emerald-500 text-xs font-bold">대기중</p>
              <p className="font-bold text-slate-700">{rentalStats.available}</p>
            </div>
            <div className="text-center">
              <p className="text-red-500 text-xs font-bold">대여중</p>
              <p className="font-bold text-slate-700">{rentalStats.rented}</p>
            </div>
          </div>
        </div>
