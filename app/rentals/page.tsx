...
export default function RentalsPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([fetchMajorAndAssets(), fetchPendingRentals()]).finally(() => setLoading(false));
  }, [authLoading, user]);

  // 대여 가능 기자재
  const fetchMajorAndAssets = async () => {
    const { data: studentData } = await supabase
      .from('student').select('major_id').eq('user_id', user!.userId).maybeSingle();

    let majorName: string | null = null;
    if (studentData?.major_id) {
      const { data: majorData } = await supabase
        .from('major').select('major_name').eq('major_id', studentData.major_id).maybeSingle();
    ...
    if (data) {
      setAvailableAssets(
        data.filter(asset => {
          ...
          // 대여중·대여신청·반납신청 모두 대여 불가
          return !asset.rentals?.some((r: any) =>
            ['대여중', '대여신청', '반납신청'].includes(r.status)
          );
        })
      );
    }
  ...
  const fetchPendingRentals = async () => {
    ...
    const { data } = await supabase
      .from('rentals')
      .select('rental_id, status, rental_date, assets(asset_id, model_name, department, asset_image, category, barcode)')
      .eq('user_id', user.userId)
      .eq('status', '대여신청')
      .order('rental_date', { ascending: false });
    if (data) setPendingRentals(data);
  };
  ...
  const handleRentRequest = async (assetId: string) => {
    ...
    const { error } = await supabase.from('rentals').insert({
      asset_id: assetId, user_id: user.userId,
      status: '대여신청', rental_date: new Date().toISOString(),
    });
    setActionLoading(null);
    if (error) alert('신청 중 오류가 발생했습니다.');
    else { await Promise.all([fetchMajorAndAssets(), fetchPendingRentals()]); setActiveTab('pending'); }
  };

  // 대여 신청 취소
  const handleCancelRequest = async (rentalId: number) => {
    if (!window.confirm('대여 신청을 취소하시겠습니까?')) return;
    setActionLoading(String(rentalId));
    const { error } = await supabase.from('rentals').delete().eq('rental_id', rentalId);
    setActionLoading(null);
    if (error) alert('취소 중 오류가 발생했습니다.');
    else { await Promise.all([fetchMajorAndAssets(), fetchPendingRentals()]); }
  };
  ...
  const filteredAssets = useMemo(() =>
    selectedCategory === '전체' ? availableAssets
      : availableAssets.filter(a => (a.category || '기타') === selectedCategory),
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-4">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-4 md:p-5">
        <h1 className="text-lg md:text-xl font-bold text-slate-800">기자재 대여 신청</h1>
        ...
        {myMajor && (
          <div className="mt-1.5 flex items-center gap-1.5 flex-wrap">
            ...
            <span className="text-xs font-bold text-violet-600 bg-violet-50 px-2 py-0.5 rounded-full">{myMajor}</span>
      ...
      <div className="flex gap-1 bg-slate-100 p-1 rounded-xl w-full md:w-fit">
        ...
        <button
          ...
          className={`flex-1 md:flex-none px-4 md:px-5 py-2.5 rounded-lg text-sm font-bold transition-all ${
            activeTab === 'pending' ? 'bg-white shadow-sm text-amber-600' : 'text-slate-500 hover:text-slate-700'
          }`}
        >
          대여 신청 현황
          {pendingRentals.length > 0 && (
            <span className="ml-1.5 px-1.5 py-0.5 bg-amber-100 text-amber-600 rounded-full text-[11px] font-black">
        ...
        <div className="space-y-3">
          {filteredAssets.length === 0 ? (
            ...
            filteredAssets.map(asset => (
              <div key={asset.asset_id} className="bg-white p-4 rounded-2xl shadow-sm border border-slate-200 hover:border-sky-300 transition-colors">
                <div className="flex gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      {asset.category && <span className="text-[11px] font-bold text-slate-500 bg-slate-100 px-1.5 py-0.5 rounded">{asset.category}</span>}
                      {asset.department && <span className="text-[11px] font-bold text-sky-600 bg-sky-50 px-1.5 py-0.5 rounded">{asset.department}</span>}
                    </div>
                ...
                >
                  {actionLoading === asset.asset_id ? (
                    <span className="flex items-center justify-center gap-2">
                      <span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />신청 중...
                    </span>
                  ) : '대여 신청'}
                </button>
              </div>
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-start gap-2.5 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3">
            ...
            <p className="text-xs text-amber-700 font-medium leading-relaxed">
              대여 신청은 관리자 승인 후 확정됩니다. 승인되면 <strong>대여 현황</strong> 페이지에서 확인할 수 있습니다.
            </p>
          </div>

          {pendingRentals.length === 0 ? (
            <div className="bg-white rounded-2xl border border-dashed border-slate-200 p-10 text-center">
              <p className="font-semibold text-slate-600">대여 신청 내역이 없습니다</p>
              <p className="text-sm text-slate-400 mt-1">대여 가능 목록에서 기자재를 신청해보세요.</p>
            </div>
          ) : (
            pendingRentals.map(rental => (
              <div key={rental.rental_id} className="bg-white p-4 rounded-2xl shadow-sm border border-amber-100">
                <div className="flex gap-3">
                  <button
                    onClick={() => setPhotoModal({ open: true, url: rental.assets?.asset_image || null, name: rental.assets?.model_name || '' })}
                    className="shrink-0 w-14 h-14 rounded-xl overflow-hidden bg-slate-100 border border-slate-200 flex items-center justify-center"
                  >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="px-2 py-0.5 bg-amber-100 text-amber-700 rounded-full text-[11px] font-bold">승인 대기 중</span>
                    </div>
                    <h3 className="font-bold text-slate-800 text-sm">{rental.assets?.model_name}</h3>
                ...
                <button
                  onClick={() => handleCancelRequest(rental.rental_id)}
                  disabled={actionLoading === String(rental.rental_id)}
                  className="mt-3 w-full py-2 border border-slate-200 text-slate-500 rounded-xl text-sm font-semibold hover:bg-slate-50 hover:border-red-300 hover:text-red-500 transition disabled:opacity-50"
                >
                  {actionLoading === String(rental.rental_id) ? '처리 중...' : '신청 취소'}
                </button>
              </div>
            ))
          )}
